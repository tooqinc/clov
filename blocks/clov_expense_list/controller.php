...
/**
 * Displays a list of Clov expenses.
 */
Loader::library('clov_list_block_controller', 'clov');
class ClovExpenseListBlockController extends ClovListBlockController {
	protected $btTable = 'btClovExpenseList';
	
	public function getBlockTypeDescription() {
		return t('List Clov expenses.');
	}
	
	public function getBlockTypeName() {
		return t('Clov Expense List');
	}
	...
	public function getPageList() {
		...
		if($project) {
			$pageList->filterByAttribute('clov_expense_project', $project->getCollectionID());
		}
		
		// Filter by payer.
		$user = $this->getUser();
		if($user) {
			$pageList->filterByAttribute('clov_expense_payer', $user->getUserID());
		}
		
		// Only show clov_expense pages.
		$pageList->filterByCollectionTypeHandle('clov_expense');
	...
	public function getTitle() {
		...
		if(!empty($blockName)) {
			...
			return ClovMessageHelper::generateListBlockTitle('Expenses', array(
	...
	public function getSortByOptions() {
		$pageTypeAttributeKeys = CollectionType::getByHandle('clov_expense')->getAvailableAttributeKeys();
